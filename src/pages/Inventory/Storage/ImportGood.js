 ...
 * @Last Modified time: 2018-04-26 13:17:40
 * 导入商品
 */
...
export default class ImportGood extends Component {
    ...
    if (this.state.fileList.length === 0) {
      this.setState({
        ...
        fileList.forEach((fileL) => {
          formData.append('fileList', fileL)
        })
        formData.append('billNo', this.props.storage.billNo)
        reqwest({
          name: 'file',
          url: `${config.APIV1}/wm/WmPurchaseIn/upload`,
          method: 'post',
          headers: {
          ...
          success: (json) => {
            if (json.data) {
              ...
              this.props.dispatch({
                type: 'storageDetails/fetch',
                payload: { billNo: this.props.storage.billNo },
              })
            } else {
              this.setState({
                fileList: [],
  ...
  render() {
    const columns = [{
      ...
      title: <div style={{ color: 'red' }}>入库数量</div>,
      dataIndex: 'address',
      key: 'address',
      width: 120,
    },
