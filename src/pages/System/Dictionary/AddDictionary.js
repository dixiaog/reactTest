 ...
 * @Last Modified time: 2018-05-10 19:05:18
 * 添加字典
 */
...
import { Save } from '../../../services/system'
...
@Form.create()
export default class AddDictionary extends Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  handleSubmit = () => {
    this.props.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (types.length) {
            const typesArray = types.filter(ele => ele.dictType === values.dictType)
            Object.assign(values, { typeName: typesArray[0].typeName })
          }
