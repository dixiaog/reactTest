...
export default {
  ...
  effects: {
    ...
    *search({ payload }, { call, put, select }) {
      ...
      const billNo = yield select(state => state.goodModal.billNo)
      const page = payload && payload.page ? Object.assign(statePage, payload.page) : statePage
      const searchParam = payload && payload.searchParam ? payload.searchParam : yield select(state => state.goodModal.searchParam)
      yield put({
        type: 'changeState',
        payload: { loading: true },
      })
      const response = yield call(getGoodModal, { ...searchParam, ...page, billNo })
      yield put({
        type: 'changeState',
