...
export function getList(count, searchParam) {
    ...
    for (let i = 0; i < count; i += 1) {
        list.push({
        billNo: Math.random() * 10 > 5 ? `00-${i}` : '',
        billDate: '2015-01-01',
        relativeBillNo: `B00-${i}`,
        supplierName: `供应商-${i}`,
        warehouseName: `仓库-${i}`,
        productName: `冬款羽绒-${i}`,
        billStatus: Math.random() * 10 > 5 ? 0 : 1,
        confirmUser: '江腾',
        approveStatus: Math.random() * 10 > 5 ? 0 : 1,
        approveUserName: '知识分子',
        approveTime: '2015-01-01',
        createUser: '操作人员',
        updateTime: '2019-09-01',
        remark: '我是备注',
        })
    }
    if (searchParam) {
...
export function getStoDataList(req, res, u, b) {
    let url = u
